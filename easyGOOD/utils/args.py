...
from easyGOOD.definitions import ROOT_DIR
...
class TrainArgs(Tap):
    ...
    mm: float = None # Momentum update coefficient.
    warmup_steps: int = None # Used for warmup cosine decay scheduler
    
    linear_head_epochs: int = None # Epochs for training linear head.
    linear_head_lr: float = None # Learning rate for training linear head.
    
    eval_step: int = None # after how many epochs we will evaluate the pre-trained model
    best_linear_head: bool = None

    scheduler: str = None # specify the scheduler
    save_checkpoint: bool = None # save checkpoint or not
...
class DatasetArgs(Tap):
    ...
    ood_train_set: bool = None # select some ood trainig set from testing set for training OOD linear head
    ood_split_fold: int = None
...
class ModelArgs(Tap):
    ...
    use_bn: bool = None # Use Batch normalization ot not
    last_activation: bool = None # the last layer contains relu or not
    
    activation: str = None # Activation function.
    tau: float = None # Temperature for InfoNCE loss.
    queue_size: int = None # the size of dynamic queue.
    
    encoder_name: str = None # Specify the type of encoder
    load_checkpoint: bool = None # Load checkpoint or not
    
    # params for REGCL
    num_clusters: int = None # specify the number of clusters for REGCL
    prototypes_lr: float = None # lr for the inner loop
    prototypes_iters: int = None # the number of inner iteration
    cmi_coefficient: float = None # the coffecient of conditional mutual information
...
class AugArgs(Tap):
    mask_feat1: float = None
    mask_feat2: float = None
    mask_edge1: float = None
    mask_edge2: float = None
    ad_aug: bool = None
    arcl: bool = None
    num_views: int = None
...
class CommonArgs(Tap):
    ...
    aug: AugArgs = None
    evaluation: EvalArgs = None

    def __init__(self, argv):
        super(CommonArgs, self).__init__()
        self.argv = argv

        from easyGOOD.utils.metric import Metric
    ...
    def process_args(self) -> None:
        ...
        self.aug = AugArgs().parse_args(args=self.argv, known_only=True)
        self.evaluation = EvalArgs().parse_args(args=self.argv, known_only=True)
